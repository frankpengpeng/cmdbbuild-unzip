...
public class WorkflowApiImpl extends AbstractServiceClientImpl implements WokflowApi {
    ...
    public FlowData get(String planId, String walkId) {
        ...
        return SimpleFlowData.builder().withInstanceId(walkId).withAttributes(attributes).withStatus(status).build();
    }
    ...
    public WokflowServiceWithPlanVersionInfo uploadPlanVersion(String processId, InputStream data) {
        ...
        HttpEntity multipart = MultipartEntityBuilder.create()
                ...
                .build();
        JsonNode response = post(format("processes/%s/versions", encodeUrlPath(processId)), multipart).asJackson();
    ...
    @Override
    public void uploadPlanVersionAndMigrateProcess(String processId, InputStream data) {
        checkNotBlank(processId, "process id cannot be null");
        checkNotNull(data, "data param cannot be null");
        HttpEntity multipart = MultipartEntityBuilder.create()
                .addBinaryBody("file", data, ContentType.APPLICATION_OCTET_STREAM, "file.xpdl")
                .build();
        post(format("processes/%s/migration", encodeUrlPath(processId)), multipart);
    }

    @Override
    public void migrateProcess(String processId) {
        checkNotBlank(processId, "process id cannot be null");
        post(format("processes/%s/migration", encodeUrlPath(processId)), "");
    }
