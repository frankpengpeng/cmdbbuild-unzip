...
public class ReportApiImpl extends AbstractServiceClientImpl implements ReportApi {
	...
	public ReportInfo createReport(ReportInfo reportInfo, List<Pair<String, byte[]>> files) {
		...
		files.forEach((f) -> {
			builder.addBinaryBody("file", listenUpload(format("report file %s upload", f.getKey()), new ByteArrayInputStream(f.getValue())), ContentType.APPLICATION_OCTET_STREAM, f.getKey());
		});
	...
	@Override
	public void uploadReportTemplate(long reportId, List<Pair<String, byte[]>> files) {
		MultipartEntityBuilder builder = MultipartEntityBuilder.create();
		files.forEach((f) -> {
			builder.addBinaryBody("file", listenUpload(format("report file %s upload", f.getKey()), new ByteArrayInputStream(f.getValue())), ContentType.APPLICATION_OCTET_STREAM, f.getKey());
		});
