...
public class CardAdapter {
	...
	public CardAdapter(final DataView dataView, final LookupRepository lookupStore) {
		this.dataView = dataView;
		this.lookupStore = lookupStore;
	}

	public void resolveAttributes(final Card card) {
		final EntryType entryType = dataView.findClasse(card.getClassName());
			...
			attributeType.accept(new ForwardingAttributeTypeVisitor() {
				...
				public void visit(final LookupAttributeType attributeType) {
					...
					if (isNotBlank(value) && isNumeric(value)) {
						...
						final Iterable<Lookup> lookupList = lookupStore.getAll();
