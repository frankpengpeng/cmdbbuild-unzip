...
import org.cmdbuild.logic.data.QueryOptionsImpl.QueryOptionsBuilder;
...
class GuestFilter {
	...
	public GuestFilter(AuthenticationStore authenticationStore, DataView dataView) {
		this.userType = authenticationStore.getType();
		this.login = authenticationStore.getLogin();
		this.dataView = dataView;
	}

	public QueryOptionsImpl apply(final Classe target, final QueryOptionsImpl queryOptions) {
		final QueryOptionsBuilder queryOptionsBuilder = QueryOptionsImpl.builder().clone(queryOptions);
...
//					private final Store<Metadata> store = Stores.nullOnNotFoundRead(_store);
					...
					public void visit(final ReferenceAttributeType attributeType) {
						 ...
						 */
						try {
							final JSONObject original = CmdbFilterUtils.toJsonObject(queryOptions.getFilter());
					...
					private JSONObject originalWithAddition(final JSONObject original, final Attribute attribute)
						...
						final FilterElementGetter filterElementGetter = new FilterElementGetter() {
							...
							private Long idValue(final Entry<String, String> classAndAttribute) {
								final Classe targetClass = dataView.findClasse(classAndAttribute.getKey());
