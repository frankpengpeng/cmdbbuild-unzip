...
public class LookupTypeValuesWsV2 {
    ...
    public Object readMany(@PathParam("lookupTypeId") String lookupTypeId, @QueryParam(LIMIT) Integer limit, @QueryParam(START) Integer offset, @QueryParam(FILTER) String filterStr, @QueryParam(ACTIVE) @DefaultValue(TRUE) Boolean activeOnly) {
        ...
        PagedElements<Lookup> lookups = activeOnly ? lookupService.getActiveLookup(decodeIfHex(lookupTypeId), offset, limit, filter) : lookupService.getAllLookup(decodeIfHex(lookupTypeId), offset, limit, filter);
    ...
    private Object serializeResponse(Lookup lookup) {
        return map(
                "code", lookup.getCode(),
