...
public class CardLockWs {
    ...
    public CardLockWs(LockService lockService, SessionService sessionService) {
    ...
    public Object createLock(@PathParam("classId") String classId, @PathParam("cardId") Long cardId) {
        ...
        LockService.LockResponse lockResponse = lockService.aquireLock(itemIdFromCardId(cardId));
        if (lockResponse.isAquired()) {
            return response(serializeLock(lockResponse.aquired()));
        } else {
