...
@Path("{a:processes|classes}/{classId}/{b:instances|cards}/{cardId}/emails/{emailId}/attachments")
...
public class CardEmailAttachmentWs {//note: duplicate code from AttachmentWs; would be great if it could be possible to merge CardEmailAttachmentWs and AttachmentWs
	...
	@Produces(APPLICATION_JSON)
	public Object create(@PathParam("emailId") Long emailId, @Multipart(value = ATTACHMENT, required = false) @Nullable WsAttachmentData attachment, @Multipart(FILE) DataHandler dataHandler) throws IOException {
	...
	@Path(EMPTY)
	@Produces(APPLICATION_JSON)
	public Object copyFrom(@PathParam("emailId") Long emailId, @Nullable WsAttachmentData attachment, @QueryParam("copyFrom_class") String sourceClassId, @QueryParam("copyFrom_card") Long sourceCardId, @QueryParam("copyFrom_id") String sourceAttachmentId) throws IOException {
		return service.copyFrom(EMAIL_CLASS_NAME, emailId, attachment, sourceClassId, sourceCardId, sourceAttachmentId);
	}

	@GET
	@Path(EMPTY)
	public Object read(@PathParam("emailId") Long emailId) {
		return service.read(EMAIL_CLASS_NAME, emailId);
	}

	@GET
	@Path("{" + ATTACHMENT_ID + "}/")
	public Object read(@PathParam("emailId") Long emailId, @PathParam(ATTACHMENT_ID) String attachmentId) {
		return service.read(EMAIL_CLASS_NAME, emailId, attachmentId);
	}
