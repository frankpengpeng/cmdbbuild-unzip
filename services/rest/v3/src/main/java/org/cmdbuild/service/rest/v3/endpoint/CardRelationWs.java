...
public class CardRelationWs {
    ...
    public CardRelationWs(UserDomainService domainService, DaoService dao, CardWsSerializationHelper helper) {
    ...
    @GET
    @Path(EMPTY)
    public Object read(@PathParam(CLASS_ID) String className, @PathParam(CARD_ID) Long cardId, @QueryParam(LIMIT) Long limit, @QueryParam(START) Long offset, @QueryParam(DETAILED) @DefaultValue(FALSE) Boolean detailed) {
        PagedElements<CMRelation> relations = domainService.getUserRelationsForCard(className, cardId, DaoQueryOptionsImpl.builder().withPaging(offset, limit).build());
