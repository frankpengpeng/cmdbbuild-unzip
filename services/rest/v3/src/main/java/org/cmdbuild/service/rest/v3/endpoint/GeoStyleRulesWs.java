...
public class GeoStyleRulesWs {
    ...
    public GeoStyleRulesWs(GisStyleRulesService rulesService, GisService gisService) {
    ...
    public Object readAll(@PathParam(CLASS_ID) String classId, @QueryParam(FILTER) String filterStr, @QueryParam(LIMIT) Integer limit, @QueryParam(START) Integer offset) {
        ...
        if (equal(classId, "_ANY")) {
            list = rulesService.getAll();
        } else {
            list = rulesService.getForClass(classId);
        }
    ...
    @POST
    @Path("tryRules")
    public Object applyRules(@PathParam(CLASS_ID) String classId, WsRulesetData data, @QueryParam("cards") @Nullable String cards) {
        Set<Long> cardIds = isBlank(cards) ? null : emptyToNull(Splitter.on(",").splitToList(cards).stream().map(CmConvertUtils::toLong).collect(toSet()));
    ...
    private GisStyleRulesetImplBuilder dataToRules(WsRulesetData data) {
        return GisStyleRulesetImpl.builder()
                ...
                .withGisAttribute(gisService.getLayerByClassAndName(data.owner, data.attribute))
                .withParams(b -> b.withAnalysisType(parseEnumOrNull(data.analysistype, GisStyleRulesetAnalysisType.class)))
