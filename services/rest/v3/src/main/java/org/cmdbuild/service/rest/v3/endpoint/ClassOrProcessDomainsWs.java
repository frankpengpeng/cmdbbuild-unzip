...
public class ClassOrProcessDomainsWs {
    ...
    public Object getDomains(@HeaderParam(VIEW_MODE_HEADER_PARAM) String viewMode, @PathParam("classId") String classId, @QueryParam(DETAILED) @DefaultValue(FALSE) Boolean includeFullDetails) {
        ...
        List<String> domainOrder = classe.getMetadata().getDomainOrder();
        List<Domain> domains = isAdminViewMode(viewMode) ? domainService.getUserDomains() : domainService.getActiveUserDomains();
        domains = domains.stream().filter(d -> d.isDomainForClasse(classe)).sorted((a, b) -> {

            int ia = domainOrder.indexOf(a.getName()), ib = domainOrder.indexOf(b.getName());
            return Integer.compare(ia < 0 ? Integer.MAX_VALUE : ia, ib < 0 ? Integer.MAX_VALUE : ib);

        }).collect(toList());
        return response(domains.stream().map(includeFullDetails ? domainSerializationHelper::serializeDetailedDomain : domainSerializationHelper::serializeBasicDomain).collect(toList()));
    }
