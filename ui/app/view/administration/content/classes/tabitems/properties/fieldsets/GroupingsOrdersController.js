Ext.define('CMDBuildUI.view.administration.content.classes.tabitems.properties.fieldsets.GroupingsOrdersFieldsetController', {
    ...
    onAddGroupClick: function (grid, rowIndex, colIndex, button, event, record, rowEl) {
        var newStore = grid.up('administration-content-classes-view').getViewModel().getStore('attributeGroupsStoreNew');
        var store = grid.up('administration-content-classes-view').getViewModel().getStore('attributeGroupsStore');
        if(!record.get('name')){
        ...
        store.add(record);
        newStore.add({});
        grid.up('administration-content-classes-view').down('#groupingsAttributesGrid').view.grid.getView().refresh();
    }
});