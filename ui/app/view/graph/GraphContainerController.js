Ext.define('CMDBuildUI.view.graph.GraphContainerController', {
    ...
    */
    ...
    compoundCytoscape: function (id, type, description, code, records) {
        var clusteringThreshold = 5;//CMDBuildUI.util.helper.Configurations.get(CMDBuildUI.model.Configuration.gis.relgraph.clusteringThreshold);
        var memoryNodeDataStructure = this.getViewModel().get('memoryNodeDataStructure');
    ...
    mapNode_2_nodes: function (record, parentId) {
        ...
        if (relationId.includes('compound')) {
            description = Ext.String.format('{0} ({1})',
                record.get('_destinationType').replace('compound_', ''),
    ...
    mapNode_2_edges: function (record, id) {
        ...
        var sourceDescription = node.data('description');
        var sourceType = node.data('_type');
        ...
        var descriptionDirect = domain.get('_descriptionDirect');
        var descriptionInverse = domain.get('_descriptionInverse');
        return {
            id: relationId,
            source: id,
            ...
            descriptionDirect: descriptionDirect,
            descriptionInverse: descriptionInverse,
    ...
    applyNavigationTree: function (records, parentTreeId, parentTreeClass) {
        var navTreeName = this.getView().down('graph-topmenu-topmenu').getViewModel().get('lastCheckedValue');
