Ext.define('CMDBuildUI.components.administration.grid.plugin.FormInRowWidget', {
    ...
    onItemCreated: function (grid, record, scope) {
        ...
        this.gridReload(grid, record, gridParams, scope, function () {
            ...
            Ext.asap(function (_grid, _storeItem, _index) {
                ...
                grid.setSelection(_storeItem);
                grid.ensureVisible(_storeItem);
    ...
    onItemRemoved: function (grid, record, scope) {
        ...
        this.gridReload(grid, record, {}, scope, function () {
    ...
    onItemUpdated: function (grid, record, scope, ignoreGridParams) {
        ...
        var isExpanded = me.isRecordNodeExpanded(grid.down('tableview'), record);

        this.gridReload(grid, record, {}, scope, function () {
            var index = store.findExact("_id", record.getId());
            var storeItem = store.getAt(index);
            if (isExpanded) {
                me.view.fireEventArgs('togglerow', [grid, storeItem, index]);
            } else {
