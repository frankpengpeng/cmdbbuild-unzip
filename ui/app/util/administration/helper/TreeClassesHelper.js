Ext.define("CMDBuildUI.util.administration.helper.TreeClassesHelper", {
     ...
     */
    appendClasses: function (withCheckbox, activeItems, withIcon, expanded) {
     ...
     */
    appendProcesses: function (withCheckbox, activeItems, withIcon, expanded) {
        var deferred = new Ext.Deferred();
    ...
    privates: {
        ...
        sortByDescription: function (a, b) {
          ...
          */
        getRecordsAsSubmenu: function (withCheckbox, activeItems, withIcon, expanded, records, menutype, parentname) {
            var output = [];
            var me = this;
            ...
            for (var i = 0; i < frecords.length; i++) {
                ...
                var menuitem = {
                    menutype: menutype,
                    index: i,
                    checked: (withCheckbox) ? activeItems.indexOf(record.name) === -1 ? false : true : undefined,
                    bind: {
                ...
                switch (menutype) {
                    case CMDBuildUI.model.administration.MenuItem.types.klass:
                        ...
                        menuitem.iconCls = withIcon ? 'x-fa fa-file-text-o' : null;
                        if (record.prototype) {
                            menuitem.iconCls = withIcon ? 'cmdbuildicon-classparent' : null;
                            menuitem.expanded = expanded;
                            menuitem.leaf = false;
                            menuitem.children = me.getRecordsAsSubmenu(withCheckbox, activeItems, withIcon, expanded, records, menutype, record.name).sort(this.sortByDescription);
                        }
                        break;

                    case CMDBuildUI.model.administration.MenuItem.types.process:

                        menuitem.iconCls = withIcon ? 'x-fa fa-cog' : null;
                        if (record.prototype) {
                            menuitem.leaf = false;
                            menuitem.expanded = expanded;
                            menuitem.iconCls = withIcon ? 'x-fa fa-cogs' : null;
                            menuitem.children = me.getRecordsAsSubmenu(withCheckbox, activeItems, withIcon, expanded, records, menutype, record.name).sort(this.sortByDescription);
                        }
                        break;
