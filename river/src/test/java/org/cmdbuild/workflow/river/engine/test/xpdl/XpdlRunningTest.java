...
import org.cmdbuild.workflow.river.engine.task.scriptexecutors.GroovyScriptServiceImpl;
...
public class XpdlRunningTest {
    ...
    private final TaskScriptExecutorService taskScriptExecutorService = new TaskScriptExecutorServiceImpl(new GroovyScriptServiceImpl(), (t) -> null);
    ...
    public void init() throws IOException {
        ...
        flowService = new RiverFlowServiceImpl(planRepository, flowRepository, taskService);
...
//        }
        assertEquals(1, flow.getTaskCount());
        checkAndCompleteTask("FinalEvaluation_user");
...
//        checkAndCompleteNextTask("Process_requestforchange_act2_noop", RiverTask::isNoop);
        ...
        assertEquals(FlowStatus.COMPLETE, flow.getStatus());
    }
...
//		checkAndCompleteNextTask("ComplexProcessOne_wp1_act1_noop", RiverTask::isNoop);
        assertEquals(1, flow.getTaskCount());
        checkAndCompleteTask("StepOne_user");
...
//            List<RiverLiveTask> tasks = asList(taskService.popTask(), taskService.popTask());
        ...
        checkAndCompleteTask("StepTwo_user");
        assertEquals(1, flow.getTaskCount());
//            assertEquals(0,flow.getTaskCount());
        checkAndCompleteTask("StepThree_user");
//        }

//        checkAndCompleteNextTask("ComplexProcessOne_wp1_act11_noop", RiverTask::isNoop);
        assertEquals(2, flow.getTaskCount());
        checkAndCompleteTask("StepFive_user");
//            assertEquals(1,flow.getTaskCount());
////            assertEquals(0,flow.getTaskCount());
//            checkAndCompleteTask( "StepThree_user");
//        {
...
//        }
        ...
        assertEquals(1, flow.getTaskCount());
        checkAndCompleteTask("StepFour_user");

        assertEquals(1, flow.getTaskCount());
        checkAndCompleteTask("StepSix_user");
...
//    }
    private void checkAndCompleteTask(String expectedTaskId) {
        ...
        flow = flowService.completedTask(new CompletedTaskImpl(new LiveTaskImpl(flow, task)));
//        assertEquals(1, flow.getTaskCount());
//        checkAndCompleteTask(new LiveTaskImpl(flow, task), expectedTaskId, taskTypeCheck);
    }
//
