...
import static org.cmdbuild.workflow.river.engine.utils.FlowGraphUtils.getAllParallelStepsConvergingOnThisStep;
    ...
    private class FlowAdvanceOperation {
        ...
        public RiverFlow processAndReturnFlow() {
            ...
            if (flow.getTasks().isEmpty()) {
                flow = RiverFlowImpl.copyOf(flow).withFlowStatus(RiverFlow.FlowStatus.COMPLETE).build();
        ...
        public FlowAdvanceOperation enterStep(Step step) {
            ...
            flow = RiverFlowImpl.copyOf(flow)
                    .withFlowStatus(RiverFlow.FlowStatus.RUNNING)
