...
@Component
public class PlanServiceRiverImpl implements PlanServiceDelegate {
	...
	public PlanServiceRiverImpl(ExtendedRiverPlanRepository planRepository, TaskDefinitionConversionService taskToTaskDataService) {
	...
	private RiverTask findUserTask(RiverPlan plan, Step step) { //TODO move this into river plan parsing
		...
		if (task.isUser()) {
			...
			checkArgument(outgoingHandler instanceof FixedOutgoingHandler);
