...
//            }
    ...
    private class TaskConverter {
        ...
        private List<WidgetData> getTaskWidgets() {
            return entriesForTask().filter((e) -> isWorwflowWidgetType(e.getKey())).map(this::parseTaskWidget).collect(toList());
        }

        private WidgetData parseTaskWidget(Entry<String, String> entry) {
            try {
                return toWidgetData(format("river_proc_%s_task_%s_", task.getPlanId(), task.getId()), entry.getKey(), entry.getValue());
            } catch (Exception ex) {
                logger.debug("error processing widget id =< {} > for plan = {} task = {} with data = \n\n{}\n", entry.getKey(), task.getPlanId(), task.getId(), entry.getValue());
                throw new WorkflowException(ex, "error processing widget for key =< %s > value =< %s >", entry.getKey(), abbreviate(entry.getValue()));
            }
        }
