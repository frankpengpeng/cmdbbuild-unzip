...
import static org.cmdbuild.workflow.core.dao.FlowCardRepositoryImpl.buildFlowCardCode;
import static org.cmdbuild.workflow.core.dao.FlowCardRepositoryImpl.isSystemAttrAndShouldBeSkipped;
...
import org.cmdbuild.workflow.river.dao.converters.FlowConversionMode;
import static org.cmdbuild.workflow.river.utils.FlowDataSerializerUtils.RIVER_FLOW_ATTRS_FROM_FLOW_DATA;
import static org.cmdbuild.workflow.river.utils.FlowDataSerializerUtils.RIVER_FLOW_STATUS_ATTR;
...
import static org.cmdbuild.workflow.river.dao.converters.TaskDefinitionConversionServiceImpl.getTaskPerformerForUserOrNull;
import static org.cmdbuild.utils.lang.CmCollectionUtils.listOf;
import static org.cmdbuild.workflow.river.dao.converters.TaskDefinitionConversionServiceImpl.getFirstNonAdminPerformerOrNull;
...
public class RiverFlowConversionServiceImpl implements RiverFlowConversionService {
    ...
    private Flow doCopyRiverFlowDataToCard(Flow flowCard, RiverFlow riverFlow) {
        ...
        List<FlowActivity> flowActivities = listOf(FlowActivity.class).accept(l -> {
            flowCard.getFlowActivities().stream().filter((a) -> newTaskIds.contains(a.getDefinitionId())).map(t -> {
                ...
                return new FlowActivityImpl(t.getInstanceId(), t.getDefinitionId(), taskPerformer);
            }).forEach(l::add);
        ...
        return FlowImpl.copyOf(flowCard)
                .withCard(CardImpl.copyOf(flowCard).addAttributes(attributesToUpdate).addAttributes(map(
                        ATTR_CODE, buildFlowCardCode(transform(flowActivities, FlowActivity::getDefinitionId), flowCard.getType()),//TODO check this 
                        FLOW_ATTR_DATA, serializedFlowData
                )).build())
    ...
    private RiverFlow flowCardToRiverFlow(RiverPlan riverPlan, Flow flowCard, FlowConversionMode mode) {
        ...
        switch (mode) {
            case CM_FULL:
                data = deserializeFlowData(flowCard.getString(FLOW_ATTR_DATA), riverPlan);
                break;
            case CM_LEAN:
                data = deserializeFlowData(flowCard.getString(FLOW_ATTR_DATA), riverPlan, in(RIVER_FLOW_ATTRS_FROM_FLOW_DATA));
                break;
            default:
        ...
        return RiverFlowImpl.builder()
                ...
                .withPlan(riverPlan)
                .withFlowStatus(convert(data.get(RIVER_FLOW_STATUS_ATTR), RiverFlow.FlowStatus.class))
    ...
    private static FlowStatus riverStatusToFlowStatus(RiverFlow.FlowStatus status) {
        switch (status) {
            case COMPLETE:
