...
public interface ProcessRepository {
    ...
    Process getPlanClasseByProviderAndPlanId(String provider, String planId);

    @Nullable
    Process getPlanClasseOrNull(String classId);

    List<Process> getAllPlanClasses();
    ...
    default List<Process> getActivePlanClasses() {
        return getAllPlanClasses().stream().filter((Process input) -> input.isActive()).collect(toList());
    }
