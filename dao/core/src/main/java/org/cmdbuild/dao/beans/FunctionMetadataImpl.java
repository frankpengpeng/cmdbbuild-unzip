...
public class FunctionMetadataImpl extends EntryTypeMetadataImpl implements FunctionMetadata {
    ...
    private final Set<StoredFunction.Category> categories;
...
//	private final Map<String, Object> metadata;
    ...
    public FunctionMetadataImpl(Map<String, String> map) {
        ...
        categories = set(Splitter.on(",").trimResults().omitEmptyStrings().splitToList(defaultString(map.get(CATEGORIES))).stream().map(StoredFunction.Category::of).collect(toList()));

//		metadata = map(CATEGORIES, categories, MASTERTABLE, masterTable, TAGS, tags);
    }

    @Override
    public Collection<StoredFunction.Category> getCategories() {
        return categories;
    }
