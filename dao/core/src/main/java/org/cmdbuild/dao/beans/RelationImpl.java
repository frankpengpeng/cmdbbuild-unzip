...
import static org.cmdbuild.dao.beans.RelationUtils.rotateRelationWithSource;
...
public class RelationImpl implements CMRelation {
    ...
    private RelationImpl(RelationImplBuilder builder, Map<String, Object> attributes) {
        ...
        if (sourceCard instanceof Card) {//TODO improve this
            checkArgument(type.isDomainForSourceClasse(((Card) sourceCard).getType()), "invalid source card = %s for domain = %s", sourceCard, type);
        }
        if (targetCard instanceof Card) {//TODO improve this
            checkArgument(type.isDomainForTargetClasse(((Card) targetCard).getType()), "invalid target card = %s for domain = %s", targetCard, type);
        }
    }
    ...
    public DateTime getBeginDate() {//TODO move to java 8 time
        return new DateTime(checkNotNull(beginDate, "no begin date for this relation (new relation)").toInstant().toEpochMilli());
    }
    ...
    public static class RelationImplBuilder implements Builder<RelationImpl, RelationImplBuilder> {
        ...
        public RelationImplBuilder withSourceDescription(String desc) {
            return this.addAttribute(ATTR_DESCRIPTION1, desc);
        }

        public RelationImplBuilder withTargetDescription(String desc) {
            return this.addAttribute(ATTR_DESCRIPTION2, desc);
        }

        public RelationImplBuilder withSourceCode(String desc) {
            return this.addAttribute(ATTR_CODE1, desc);
        }

        public RelationImplBuilder withTargetCode(String desc) {
            return this.addAttribute(ATTR_CODE2, desc);
        }
        ...
        public RelationImplBuilder addAttribute(String key, @Nullable Object value) {
