...
public class ClasseHierarchyServiceImpl implements ClasseHierarchyService {
	...
	public ClasseHierarchyServiceImpl(ClasseRepository classeRepository, DaoEventService eventService, CacheService cacheService) {
		...
		eventService.getEventBus().register(new Object() {
			...
			public void handleAttributeModifiedEvent(AttributesModifiedEvent event) {
				if (event.getOwner() instanceof Classe) {
					eventService.post(ClassStructureChangedEvent.affecting(transform(getClasseHierarchy((Classe) event.getOwner()).getDescendantsAndSelf(), Classe::getOid)));
				}
			}
	...
	@Override
	public ClasseHierarchy getClasseHierarchy(Classe classe) {
		return checkNotNull(allHierarchyCache.get(this::doGetClasseHierarchy).get(classe.getName()), "class hierarchy not found for class = %s", classe);
	}
