...
BEGIN
	...
	FOR _class, _trigger, _args IN 
			...
			SELECT tgrelid::regclass, tgname, args FROM _triggers WHERE tgrelid::regclass IN (SELECT _cm3_class_list()) LOOP
	...
	FOR _class, _trigger, _args IN 
		...
		RAISE NOTICE 'upgrade _cm_trigger_fk for class = % with name = % args = %, new name = %', _class, _trigger, _args, _new_name;
		EXECUTE format('DROP TRIGGER "%s" ON %s', _trigger, _class);
	...
	FOR _class, _trigger, _args IN 
		...
		RAISE NOTICE 'upgrade _cm_trigger_update_relation for class = % with name = % args = %, new name = %', _class, _trigger, _args, _new_name;
		EXECUTE format('DROP TRIGGER "%s" ON %s', _trigger, _class);
	...
	FOR _class, _trigger, _args IN 
			...
			SELECT tgrelid::regclass, tgname, args FROM _triggers WHERE tgrelid::regclass IN (SELECT _cm3_domain_list()) LOOP
		_otherclass = _args[2]::regclass;
		_attr = _args[1];
		...
		RAISE NOTICE 'upgrade _cm_trigger_update_reference for relation = % with name = % args = %, new name = %', _class, _trigger, _args, _new_name;
		EXECUTE format('DROP TRIGGER "%s" ON %s', _trigger, _class);
