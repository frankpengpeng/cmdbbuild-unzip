...
BEGIN
	IF EXISTS (SELECT * FROM information_schema.schemata WHERE schema_name = 'gis') THEN
		...
		FOR _gis_master, _gis_master_attr IN SELECT "Owner", "Code" FROM "_GisAttribute" WHERE "Status" = 'A' LOOP
			EXECUTE format('INSERT INTO _cm3_gis_patch_aux(id,usr,begindate,masterclass,master,masterattr,geometry) SELECT "Id","User","BeginDate",%L,"Master",%L,gis.ST_AsEWKB("Geometry") FROM gis."Detail_%s_%s"',
				_gis_master, _gis_master_attr, _cm3_utils_regclass_to_name(_gis_master), _gis_master_attr);
-- 			EXECUTE format('DROP TRIGGER IF EXISTS "_Constr_%s_%s_Master" ON %s',_cm3_utils_regclass_to_name(_gis_master), _gis_master_attr, _gis_master); not required, already removed in baseclass patch
		END LOOP;
