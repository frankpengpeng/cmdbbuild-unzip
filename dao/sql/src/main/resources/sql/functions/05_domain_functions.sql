...
BEGIN
	...
	THEN
		RAISE EXCEPTION 'CM_FORBIDDEN_OPERATION: invalid comment features on domain = % current features = "%" new features = "%"', _domain, _cm3_comemnt_from_jsonb(_current_features), _cm3_comemnt_from_jsonb(_features);
	END IF;
...
CREATE OR REPLACE FUNCTION _cm3_domain_source_check(_domain regclass, _source_class regclass) RETURNS void AS $$ BEGIN
    IF _source_class IS NULL OR NOT EXISTS (SELECT c FROM _cm3_class_list_descendant_classes_and_self_not_superclass(_cm3_utils_name_to_regclass(_cm3_class_comment_get(_domain, 'CLASS1'))) c WHERE c = _source_class) THEN --TODO check also disabled classes
...
CREATE OR REPLACE FUNCTION _cm3_domain_target_check(_domain regclass, _target_class regclass) RETURNS void AS $$ BEGIN
    IF _target_class IS NULL OR NOT EXISTS (SELECT c FROM _cm3_class_list_descendant_classes_and_self_not_superclass(_cm3_utils_name_to_regclass(_cm3_class_comment_get(_domain, 'CLASS2'))) c WHERE c = _target_class) THEN --TODO check also disabled classes
