...
BEGIN
	...
	PERFORM _cm3_attribute_create(_class, 'Master', 'bigint', format('NOTNULL: true|UNIQUE: true|FKTARGETCLASS: %s', _cm3_utils_regclass_to_name(_master)));
    EXECUTE format('ALTER TABLE %s ADD COLUMN "Geometry" gis.geometry(%s,900913) NOT NULL', _class, _type);
	RETURN _class;
...
BEGIN
    EXECUTE format('INSERT INTO %s ("Master", "Geometry") VALUES (%L, gis.st_geomfromtext(%L, 900913)) ON CONFLICT ("Master") DO UPDATE SET "Geometry" = EXCLUDED."Geometry"', _gis_table, _card, _value_astext);
END $$ LANGUAGE PLPGSQL;
...
CREATE OR REPLACE FUNCTION _cm3_gis_value_get(_class regclass, _attr varchar, _card bigint) RETURNS varchar AS $$ DECLARE
    ...
    _value varchar;
BEGIN
    EXECUTE format('SELECT gis.st_astext("Geometry") FROM %s WHERE "Master" = %s', _gis_table, _card) INTO _value;
    RETURN _value;
END $$ LANGUAGE PLPGSQL;
