...
public class FunctionManagerImpl implements FunctionManager {
    ...
    private void upgradeFunction(SqlFunction function) {
        ...
        try {
            ...
            cardRepository.update(function);
        } catch (Exception ex) {
    ...
    private List<SqlFunction> getPendingFunctions() {
        ...
        Map<String, SqlFunction> appliedFunctions = map(transformValues(cardRepository.getFunctions(), f -> firstNotNull(availableFunctionsByHash.get(f.getHash()), f))); //TODO improve this (handling of multiple function versions with different version requirements)
    ...
    private String buildFunctionUpgradeSqlScript(SqlFunction function) {
        return format("\nDO $_cm_function_upgrade_$ BEGIN\n\n%s\n\nEXCEPTION WHEN invalid_function_definition THEN\nDROP FUNCTION IF EXISTS %s;\n\n%s\n\nEND $_cm_function_upgrade_$ LANGUAGE PLPGSQL;\n",
                function.getFunctionDefinition(), function.getSignature(), function.getFunctionDefinition());
    }
