...
public class EcqlServiceImpl implements EcqlService {
	...
	private class EcqlProcessor {
		...
		private Object evalJs(String expr) {
			...
			jsCode = getEasytemplateProcessorForJs().processExpression(jsCode);
			return evalJavascriptCode(format("var context = %s;\n\nvar result;\n\nwith(context){\n\tresult = %s;\n};\n\nresult;", jsContext, jsCode));
		}

		private EasytemplateProcessor getEasytemplateProcessorForJs() {
			return EasytemplateProcessorImpl.builder().withResolvers(transformValues(getEasytemplateProcessor().getResolvers(), (f) -> compose((x) -> {
				if (x == null) {
					return null;
				} else if (x instanceof String) {
					return format("'%s'", StringEscapeUtils.escapeEcmaScript((String) x));
				} else {
					return x;
				}
			}, f))).build();
