...
public class PostgresDriverAutoconfigureHelperServiceImpl implements PostgresDriverAutoconfigureHelperService {
	...
	public void autoconfigurePostgresDriver() {
		...
		try {
			if (directoryService.hasContainerDirectory() && directoryService.hasWebappDirectory() && equal(databaseConfiguration.getDriverClassName(), DEFAULT_DB_DRIVER_CLASS_NAME)) {
				File disabledDriverJarFile = list(directoryService.getWebappLibDirectory().listFiles()).stream().filter(f -> f.getName().matches("postgresql-[0-9.-]+[.]jar_disabled")).collect(onlyElement());
				logger.info("configuring postgres driver from file = {}", disabledDriverJarFile);
				File newDriverJarFile = new File(directoryService.getWebappLibDirectory(), disabledDriverJarFile.getName().replaceFirst("_disabled", ""));
				logger.info("copy postgres driver to file = {}", newDriverJarFile);
				Files.copy(disabledDriverJarFile, newDriverJarFile);
