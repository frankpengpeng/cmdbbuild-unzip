...
public enum DomainConverter implements Converter {
	...
	DESCRIPTION(description()) {
		...
		public TranslationObject create() {
			validate();
			final org.cmdbuild.translation.legacy.DomainDescription.Builder builder = DomainDescription
					.newInstance() //
	...
	DIRECT_DESCRIPTION(directDescription()) {
		...
		@Override
		public TranslationObject create() {
			final org.cmdbuild.translation.legacy.DomainDirectDescription.Builder builder = DomainDirectDescription
					.newInstance() //
					.withDomainName(domainName);
			if (!translations.isEmpty()) {
				builder.withTranslations(translations);
	...
	INVERSE_DESCRIPTION(inverseDescription()) {
		...
		@Override
		public TranslationObject create() {
			final org.cmdbuild.translation.legacy.DomainInverseDescription.Builder builder = DomainInverseDescription
					.newInstance() //
					.withDomainName(domainName);
			if (!translations.isEmpty()) {
				builder.withTranslations(translations);
	...
	MASTERDETAIL_LABEL(masterDetail()) {
		...
		@Override
		public TranslationObject create() {
			final org.cmdbuild.translation.legacy.DomainMasterDetailLabel.Builder builder = DomainMasterDetailLabel
					.newInstance() //
					.withDomainName(domainName);
			if (!translations.isEmpty()) {
				builder.withTranslations(translations);
