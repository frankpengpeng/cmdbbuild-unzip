...
public enum AttributeConverter implements Converter {
	...
	CLASSATTRIBUTE_DESCRIPTION(forClass(), description()) {
		...
		public TranslationObject create() {
			final org.cmdbuild.translation.legacy.ClassAttributeDescription.Builder builder = ClassAttributeDescription
					.newInstance() //
	...
	CLASSATTRIBUTE_GROUP(forClass(), group()) {
		...
		@Override
		public TranslationObject create() {
			final org.cmdbuild.translation.legacy.ClassAttributeGroup.Builder builder = ClassAttributeGroup
					.newInstance() //
					.withClassname(parentName) //
					.withAttributename(attributeName);
			if (!translations.isEmpty()) {
