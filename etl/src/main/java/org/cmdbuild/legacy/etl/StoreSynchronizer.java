...
import org.cmdbuild.legacy.etl.Synchronizer;
...
public class StoreSynchronizer implements Synchronizer {
	...
	private void doSync() {
		...
		logger.trace("source data", sourceData);

		final Map<Key, Entry> targetData = uniqueIndex(right.readAll(), BY_KEY);
		logger.trace("target data", targetData);
		...
		logger.trace("entries to be created...");
		...
		logger.trace("entries to be updated...");
		if (logger.isTraceEnabled()) {
			for (final ValueDifference<Entry> element : toUpdate.values()) {
				logger.trace("\t{} vs {}", element.leftValue(), element.rightValue());
			}
		}

		final Map<Key, Entry> toRemove = mapDifference.entriesOnlyOnRight();
		logger.trace("entries to be deleted...");
		if (logger.isTraceEnabled()) {
