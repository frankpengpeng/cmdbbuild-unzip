...
public class InternalStore implements Store {
	...
	private static abstract class Action<T> {
		...
		protected Action(final DataView dataView) {
			this.dataView = dataView;
		}
	...
	private static class Create extends Action<Void> implements TypeVisitor {
		...
		public Create(final DataView dataView, final AttributeValueAdapter attributeValueAdapter, final Entry entry) {
			super(dataView);
			this.attributeValueAdapter = attributeValueAdapter;
			this.entry = entry;
		}
	...
	private static class ReadAllByType extends Action<Iterable<Entry>> implements TypeVisitor {
		...
		public ReadAllByType(final DataView dataView, final Cache<Key, Long> cache,
		...
		public void visit(final ClassType type) {
			...
			final Classe targetType = dataView.findClasse(typeName);
			// TODO handle class not found
			final Iterable<Card> cards = from(dataView.select(anyAttribute(targetType)) //
					.from(targetType) //
					.run()) //
					.transform(toCard(targetType));
			for (final Card card : cards) {
				final Iterable<Map.Entry<String, Object>> rawValues = from(card.getRawValues()) //
	...
	private static class Update extends Action<Void> implements TypeVisitor {
		...
		public Update(final DataView dataView, final Cache<Key, Long> cache,
		...
		public void visit(final ClassType type) {
			...
			final Card card = dataView //
					.select(anyAttribute(targetType)) //
					.from(targetType) //
					.where(condition(attribute(targetType, org.cmdbuild.dao.constants.SystemAttributes.ATTR_ID), eq(id))) //
					.limit(1) //
					.skipDefaultOrdering() //
					.run() //
					.getOnlyRow() //
	...
	private static class Delete extends Action<Void> implements TypeVisitor {
		...
		public Delete(final DataView dataView, final Cache<Key, Long> cache, final Entry entry) {
			super(dataView);
			this.cache = cache;
			this.entry = entry;
		}
