...
public class CmReflectionUtils {
    ...
    public static <T> T executeMethod(Object instance, String methodName, Object... args) {
        ...
        try {
            if (args.length == 0) {
                ...
                method = stream(instance.getClass().getMethods()).filter(m -> equal(m.getName(), methodName) && m.getParameterCount() == args.length).collect(onlyElement("method not found"));
            }
    ...
    public static <T> T executeMethod(Object instance, Method method, Object... args) {
        ...
        try {
            ...
            throw runtime(inner(ex), "error executing method = %s with args = %s on instance = %s", method, list(args), instance);
        }
    }
