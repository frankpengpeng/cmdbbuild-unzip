...
import static org.cmdbuild.auth.login.AuthorityUtils.getAutoritiesForUser;
...
public class AuthenticatedUser implements Authentication, OperationUserStack {
	...
	public AuthenticatedUser(OperationUserStack operationUserStack) {
		...
		this.authorities = getAutoritiesForUser(operationUserStack).stream().map(a -> new SimpleGrantedAuthority(a)).collect(toImmutableList());
	}
